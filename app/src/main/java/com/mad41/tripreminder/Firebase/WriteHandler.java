    ...
    public static void  WriteInfireBase(List<Trip> trips , String userId) {
      ...
      //  mDatabase.removeValue();
        ...
        for(int i=0 ; i <trips.size() ; i++) {
            ...
            System.out.println("Notess from room ....." + trip.getNotes().size());
            String Notes = "";
            if (trip.getNotes().size() != 0) {
                ...
                for (int j = 0; j < noteSize; j++) {
                    ...
                    Notes = Notes + "##%" + trip.getNotes().get(j);
                }
            }
            ...
            mDatabase.child(i + 1 + "").child("TripsInfo").child("Notes").setValue(Notes);
        }


        }

       // MainScreen.deleteFireBase.stop();
